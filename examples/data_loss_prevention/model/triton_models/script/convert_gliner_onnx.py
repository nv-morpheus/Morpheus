...
def convert_bi_encoder_to_onnx(
    ...
    model_input, raw_batch = model.prepare_model_inputs(
    ...
    class BiEncoderONNXWrapper(torch.nn.Module):
        ...
        def forward(
            ...
            # Remove None values
            model_inputs = {
                k: v for k, v in model_inputs.items() if v is not None
            }
    ...
    torch.save({"labels": example_labels, "embeddings": labels_embeddings},
               labels_path)
...
def test_onnx_model(onnx_model_dir, test_texts, test_labels):
    ...
    for i, output in enumerate(onnx_results):
        start_token_idx_to_text_idx = raw_batch[
            "all_start_token_idx_to_text_idx"
        ][i]
        end_token_idx_to_text_idx = raw_batch[
...
def convert_gliner_to_onnx(
    ...
    convert_bi_encoder_to_onnx(model, onnx_model_path, example_texts,
                               example_labels)

...
class LogParsingPostProcessingStage(SinglePortStage):
    ...
    def _postprocess(self, x: MultiResponseMessage | ControlMessage):
        ...
        if isinstance(x, ControlMessage):
            ...
            infer_pdf.columns = ["doc", "start", "stop"]
            infer_pdf["confidences"] = x.tensors().get_tensor('confidences').tolist()
            infer_pdf["labels"] = x.tensors().get_tensor('labels').tolist()
            infer_pdf["token_ids"] = x.tensors().get_tensor('input_ids').tolist()

...
@click.option("--output_prefix", type=str, default="dfp-viz-duo-", help=("Prefix for output files"))
def run_pipeline(train_users,
    ...
    if (is_training):
        ...
        pipeline.add_stage(DFPVizPostprocStage(config, period="D", output_dir=output_dir, output_prefix=output_prefix))
    # Run the pipeline
    pipeline.run()

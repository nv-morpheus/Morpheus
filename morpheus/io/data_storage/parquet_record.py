...
class ParquetRecordStorage(RecordStorageInterface):
    """Class for Parquet record storage."""

    def save(self, record, data_label: str):
        """Save the provided record as a Parquet file."""

        if self._storage_type == 'filesystem':
            self._backing_source = data_label
            record.to_parquet(self._backing_source)
        elif self._storage_type == 'in_memory':
            buf = io.BytesIO()
            record.to_parquet(buf)
            self._backing_source = buf
        else:
            raise ValueError(f"Invalid storage_type '{self._storage_type}'")

    def load(self):
        """Load and return the saved Parquet record."""

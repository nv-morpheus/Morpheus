...
void start_listeners(morpheus::payload_parse_fn_t* payload_parse_fn,
                     const std::string& bind_address,
                     unsigned short port,
                     const std::string& endpoint,
                     http::verb method,
                     unsigned short num_threads)
{
    ...
    // We'll start one thread here, and the others we'll hold references to
    std::vector<std::thread> listener_threads(num_threads - 1);
    ...
    while (g_is_running)
    {
        ...
        try
        {
            ...
            http::read(socket, buffer, req);  // todo: define parser and set body_limit

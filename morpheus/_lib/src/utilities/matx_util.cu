...
{  // NOLINT
  ...
  private:
    ...
    void threshold_by_row(void* input_data, void* output_data, double threshold, const std::vector<std::size_t>& stride)
    {
        ...
        matx::tensor_t<InputT, 2> input_tensor(
            static_cast<InputT*>(input_data),
            input_shape,
    ...
    void threshold(void* input_data, void* output_data, double threshold, const std::vector<std::size_t>& stride)
    {
        ...
        matx::index_t matx_stride[2] = {static_cast<matx::index_t>(stride[0]), static_cast<matx::index_t>(stride[1])};
                                                         ...
                                                         const std::vector<int64_t>& output_shape)
{
    ...
    for (std::size_t i = 0; i < num_input_rows; ++i)
    {
        ...
        {
            cudf::type_dispatcher(cudf_type, matx_reduce_max, start, i, seq_ids[start + seq_id_offset] - output_offset);
            start = i;
        }
    }

    cudf::type_dispatcher(
        cudf_type, matx_reduce_max, start, num_input_rows, seq_ids[start + seq_id_offset] - output_offset);

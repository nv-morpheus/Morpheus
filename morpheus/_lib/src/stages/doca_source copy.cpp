...
DocaSourceStage::subscriber_fn_t DocaSourceStage::build()
{
  return [this](rxcpp::subscriber<source_type_t> output) {
    ...
    while (output.is_subscribed())
    {
      ...
      auto data_size_out_d    = rmm::device_uvector<int32_t>(packet_count, processing_stream); // new here
      auto tcp_flags_out_d    = rmm::device_uvector<int32_t>(packet_count, processing_stream); // new here
      ...
      morpheus::doca::packet_gather_kernel(
        ...
        data_size_out_d.data(), // new here
        tcp_flags_out_d.data(), // new here
        data_out_d.data(),
        processing_stream
      );
      ...
      metadata.column_names.push_back("packet_size"); // new here
      my_columns.push_back(std::move(data_size_out_d_col));    // new here  

      metadata.column_names.push_back("tcp_flags"); // new here
      my_columns.push_back(std::move(tcp_flags_out_d_col));    // new here        

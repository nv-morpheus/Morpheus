...
DeserializeStage::DeserializeStage(TensorIndex batch_size, bool ensure_sliceable_index) :
  DeserializeStageBase(base_t::op_factory_from_sub_fn(build_operator())),
...
// ************ DeserializationStage **************************** //
DeserializeControlMessageStage::DeserializeControlMessageStage(
    TensorIndex batch_size, bool ensure_sliceable_index, std::unique_ptr<DeserializeControlMessageStage::Task> task) :
  DeserializeStageBase(base_t::op_factory_from_sub_fn(build_operator())),
  m_batch_size(batch_size),
  m_ensure_sliceable_index(ensure_sliceable_index),
...
DeserializeControlMessageStage::subscribe_fn_t DeserializeControlMessageStage::build_operator()
{
    return [this](rxcpp::observable<sink_type_t> input, rxcpp::subscriber<source_type_t> output) {
        return input.subscribe(rxcpp::make_observer<sink_type_t>(
            [this, &output](sink_type_t x) {
                ...
                for (TensorIndex i = 0; i < x->count(); i += this->m_batch_size)
                {
                    const auto num_rows = std::min(i + this->m_batch_size, x->count());
                    auto window         = full_message->copy_ranges({{i, num_rows}}, num_rows - i);
                    auto message        = std::make_shared<ControlMessage>();
                    message->payload(window->meta);
                    if (m_task)
                    {
                        message->add_task(m_task->type, m_task->payload);
                    }

                    output.on_next(std::move(message));
                }
            },
            [&](std::exception_ptr error_ptr) {
                output.on_error(error_ptr);
...
DeserializeControlMessageStageInterfaceProxy::init(mrc::segment::Builder& builder,
...
{
    std::unique_ptr<DeserializeControlMessageStage::Task> task{nullptr};

    if (!task_type.is_none() && !task_payload.is_none())
    {
        task = std::make_unique<DeserializeControlMessageStage::Task>(pybind11::cast<std::string>(task_type),
                                                                      mrc::pymrc::cast_from_pyobject(task_payload));
    }

    auto stage = builder.construct_object<DeserializeControlMessageStage>(

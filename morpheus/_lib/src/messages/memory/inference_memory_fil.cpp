...
namespace morpheus {
    ...
    /****** InferenceMemoryFIL****************************************/
    InferenceMemoryFIL::InferenceMemoryFIL(size_t count, neo::TensorObject input__0, neo::TensorObject seq_ids)
            : InferenceMemory(
            count) {
    ...
    const neo::TensorObject &InferenceMemoryFIL::get_input__0() const {
    ...
    void InferenceMemoryFIL::set_input__0(neo::TensorObject input__0) {
        this->inputs["input__0"] = std::move(input__0);
    }

    const neo::TensorObject &InferenceMemoryFIL::get_seq_ids() const {
    ...
    void InferenceMemoryFIL::set_seq_ids(neo::TensorObject seq_ids) {
        this->inputs["seq_ids"] = std::move(seq_ids);
    }
    /****** InferenceMemoryFILInterfaceProxy *************************/
    std::shared_ptr<InferenceMemoryFIL>
    InferenceMemoryFILInterfaceProxy::init(cudf::size_type count, pybind11::object input__0, pybind11::object seq_ids)  {
        // Convert the cupy arrays to tensors
        return std::make_shared<InferenceMemoryFIL>(count,
                                                    std::move(CupyUtil::cupy_to_tensor(input__0)),
    ...
    neo::TensorObject InferenceMemoryFILInterfaceProxy::get_tensor(InferenceMemoryFIL &self, const std::string &name) {

 ...
 */
    ...
    struct ITensorOperations {
        ...
        virtual std::shared_ptr<ITensor>
        copy_rows(const std::vector<std::pair<TensorIndex, TensorIndex>> &selected_rows,
    ...
    struct TensorObject final {
        ...
        T read_element(const TensorIndex (&idx)[N]) const {
            ...
            CHECK(DataType::create<T>() == this->dtype())
                            << "read_element type must match array type. read_element type: '"
                            << DataType::create<T>().name()
            ...
            MRC_CHECK_CUDA(
                    cudaMemcpy(&output, static_cast<uint8_t *>(this->data()) + offset, sizeof(T),
        ...
        T read_element(const std::array<TensorIndex, N> idx) const {
            ...
            CHECK(
                    std::transform_reduce(shape.begin(), shape.end(), std::begin(idx), 1, std::logical_and<>(),
                                          std::less<>()))
                            << "Index is outsize of the bounds of the tensor. Index="
                            << detail::array_to_str(std::begin(idx), std::begin(idx) + N)
                            << ", Size=" << detail::array_to_str(shape.begin(), shape.end()) << "";
        ...
        TensorObject &operator=(const TensorObject &other) {
            ...
            DCHECK(this->bytes() == other.bytes())
                            << "Left and right bytes should be the same if all other test passed";

...
def cast_to_str(df, column_name: str, prefix: str = ""):
    if (column_name in df and df[column_name].dtype != np.dtype('O')):
        return prefix + df[column_name].astype(str)
...
def clean_column(df, column_name: str):
    if (column_name in df):
        df[column_name] = df[column_name].apply(lambda x: remove_null(x))
    return df[column_name]

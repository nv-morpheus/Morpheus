...
if hasArg upload; then
   ...
   echo -e "${y}Uploading morpheus_core conda package${x}"
...
if [[ ${NUMARGS} == 0 ]]; then
   ...
   echo -e "${r}   morpheus_core${x}"
   echo -e "${r}Exiting...${x}"
   exit 12
fi

if hasArg morpheus_core; then
   # Set GIT_VERSION to set the project version inside of meta.yaml
   export GIT_VERSION="$(get_version)"

   echo "Running conda-build for morpheus_core v${GIT_VERSION}..."
   set -x
   conda ${CONDA_COMMAND} "${CONDA_ARGS_ARRAY[@]}" ${CONDA_ARGS} ci/conda/recipes/morpheus_core
   set +x
fi

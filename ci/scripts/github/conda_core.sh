...
CI_SCRIPT_ARGS="$@"
source ${WORKSPACE}/ci/scripts/github/common_core.sh
...
rapids-logger "Building Conda Package morpheus_core"
...
${MORPHEUS_ROOT}/ci/conda/recipes/run_conda_core_build.sh morpheus_core "${CI_SCRIPT_ARGS}"

# If we didn't receive the upload argument, upload the artifact to S3
if [[ " ${CI_SCRIPT_ARGS} " =~ " upload " ]]; then
   rapids-logger "Building Conda Package morpheus_core... Done"
else

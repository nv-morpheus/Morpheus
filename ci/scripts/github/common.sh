...
_FLAGS="-B ${BUILD_DIR} -G Ninja -DCMAKE_MESSAGE_CONTEXT_SHOW=ON -DMORPHEUS_CUDA_ARCHITECTURES=RAPIDS"
_FLAGS="${_FLAGS} -DMORPHEUS_USE_CONDA=ON -DMORPHEUS_USE_CCACHE=ON"
_FLAGS="${_FLAGS} -DMORPHEUS_PYTHON_INPLACE_BUILD=OFF -DMORPHEUS_PYTHON_BUILD_STUBS=ON"
_FLAGS="${_FLAGS} -DMORPHEUS_BUILD_BENCHMARKS=ON -DMORPHEUS_BUILD_EXAMPLES=ON -DMORPHEUS_BUILD_TESTS=ON "
if [[ "${LOCAL_CI}" == "" ]]; then
    _FLAGS="${_FLAGS} -DCCACHE_PROGRAM_PATH=$(which sccache)"
fi

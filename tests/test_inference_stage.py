...
def test_convert_response(config):
    ...
    for i, s in enumerate(message_sizes):
        output_memory.append(
            ResponseMemory(count=s, tensors={"probs":full_output[sum(message_sizes[:i]):sum(message_sizes[:i]) + s, :]}))
    ...
    assert (resp.memory.get_output("probs") == full_output).all()
...
def test_convert_response_errors():
    ...
    out_msg1 = ResponseMemory(count=1, probs=cp.random.rand(1, 3))
    out_msg2 = ResponseMemory(count=1, probs=cp.random.rand(1, 3))
    ...
    out_msg1 = ResponseMemory(count=1, probs=cp.random.rand(1, 3))
    out_msg2 = ResponseMemory(count=2, probs=cp.random.rand(2, 3))
...
def test_convert_one_response():
    ...
    mem = ResponseMemory(4, probs=cp.zeros((4, 3)))

    inf = _mk_message(mess_count=4, count=4)
    res = ResponseMemory(count=4, probs=cp.random.rand(4, 3))
    ...
    res = ResponseMemory(count=3, probs=cp.array([[0, 0.6, 0.7], [5.6, 4.4, 9.2], [4.5, 6.7, 8.9]]))

    mem = ResponseMemory(2, probs=cp.zeros((2, 3)))
    mpm = InferenceStage._convert_one_response(MultiResponseMessage.from_message(inf, memory=mem), inf, res)
    assert mem.get_output('probs').tolist() == [[0, 0.6, 0.7], [5.6, 6.7, 9.2]]


def test_convert_one_response_error():
    mem = ResponseMemory(2, probs=cp.zeros((2, 2)))

...
def _build_engine(mock_langchain_agent_executor: mock.MagicMock) -> LLMEngine:
    engine = LLMEngine()
    engine.add_node("extracter", node=ExtracterNode())
    engine.add_node("chain",
                    inputs=["/extracter"],
...
def test_pipeline(config: Config, dataset_cudf: DatasetManager, mock_langchain_agent_executor: mock.MagicMock):
    input_df = dataset_cudf["filter_probs.csv"]
    expected_df = input_df.copy(deep=True)

    mock_langchain_agent_executor.arun.return_value = 'frogs'
    ...
    pipe.add_stage(LLMEngineStage(config, engine=_build_engine(mock_langchain_agent_executor)))

...
- NVIDIA Driver 525 or higher (currently Morpheus is not compatible with NVIDIA Driver versions 580+ due to a known issue with Numba refer to [#2305](https://github.com/nv-morpheus/Morpheus/issues/2305))
...
1. Choose a version
1. Download the selected version, for example for `25.10`:
    ```bash
    docker pull nvcr.io/nvidia/morpheus/morpheus:25.10-runtime
    ```
...
```bash
docker run --rm -ti --runtime=nvidia --gpus=all --net=host -v /var/run/docker.sock:/var/run/docker.sock nvcr.io/nvidia/morpheus/morpheus:25.10-runtime bash
```
...
The `./docker/run_container_release.sh` script accepts the same `DOCKER_IMAGE_NAME`, and `DOCKER_IMAGE_TAG` environment variables that the `./docker/build_container_release.sh` script does. For example, to run version `v25.10.00` use the following:

```bash
DOCKER_IMAGE_TAG="v25.10.00-runtime" ./docker/run_container_release.sh
```
...
docker run --rm -ti --gpus=all -p8000:8000 -p8001:8001 -p8002:8002 \
  nvcr.io/nvidia/morpheus/morpheus-tritonserver-models:25.10 \

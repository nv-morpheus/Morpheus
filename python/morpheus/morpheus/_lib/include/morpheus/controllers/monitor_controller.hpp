...
// Adding "\n" (new line) "\033[A" (move cursor up) and "\033[1L" (insert line)
// before each log output line. This keeps the progress bars always display as
// the last line of console output
...
// A singleton manager class that manages the lifetime of progress bars related
// to any MonitorController<T> instances Meyer's singleton is guaranteed
// thread-safe
...
public:
  ...
  operator=(ProgressBarContextManager const &) = delete;

  size_t add_progress_bar(
      const std::string &description,
      indicators::Color text_color = indicators::Color::cyan,
      indicators::FontStyle font_style = indicators::FontStyle::bold) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_progress_bars.push_back(std::move(
        initialize_progress_bar(description, text_color, font_style)));

    // DynamicProgress should take ownership over progressbars:
    // https://github.com/p-ranav/indicators/issues/134 The fix to this issue is
    // not yet released, so we need to:
    //     - Maintain the lifetime of the progress bar in m_progress_bars
    //     - Push the underlying progress bar object to the DynamicProgress
    //     container, since it accepts
    //        Indicator &bar rather than std::unique_ptr<Indicator> bar before
    //        the fix
  ...
  void display_all() {
    ...
    // To avoid display_all() being executed after calling
    // mark_pbar_as_completed() in race conditions
    if (m_is_completed) {
      return;
    }

    // A bit of hack here to make the font settings work. Indicators enables the
    // font options only if the bars are output to standard streams (see
    // is_colorized() in <indicators/termcolor.hpp>), but since we are still
    // using the ostream (m_stdout_os) that is connected to the console
    // terminal, the font options should be enabled. The internal function here
    // is used to manually enable the font display.
    m_stdout_os.iword(termcolor::_internal::colorize_index()) = 1;

    for (auto &pbar : m_progress_bars) {
      pbar->print_progress(true);
      m_stdout_os
          << termcolor::reset; // The font option only works for the current bar
      m_stdout_os << "\n" << std::flush;
    }

    // After each round of display, move cursor up ("\033[A") to the beginning
    // of the first bar
  ...
  void mark_pbar_as_completed(size_t bar_id) {
    ...
    if (!m_is_completed) {
      ...
      if (all_pbars_completed) {
        ...
        // Doing this here instead of the destructor to avoid a race condition
        // with the pipeline's
...
private:
  ProgressBarContextManager()
      : m_stdout_streambuf(std::cout.rdbuf()), m_stdout_os(m_stdout_streambuf) {
    init_log_streambuf();
  }
  ...
  std::unique_ptr<indicators::IndeterminateProgressBar>
  initialize_progress_bar(const std::string &description,
    ...
    auto progress_bar = std::make_unique<indicators::IndeterminateProgressBar>(
        indicators::option::BarWidth{10}, indicators::option::Start{"["},
        indicators::option::Fill{" "}, indicators::option::Lead{"#"},
        indicators::option::End("]"),
        indicators::option::PrefixText{description},
        indicators::option::ForegroundColor{text_color},
        indicators::option::FontStyles{
            std::vector<indicators::FontStyle>{font_style}},
        indicators::option::Stream{m_stdout_os});

    return std::move(progress_bar);
  }

  indicators::DynamicProgress<indicators::IndeterminateProgressBar>
      m_dynamic_progress_bars;
  std::vector<std::unique_ptr<indicators::IndeterminateProgressBar>>
      m_progress_bars;
  std::mutex m_mutex;

  // To ensure progress bars are displayed alongside other log outputs, we use
  // two distinct stream buffers:
  //  - Progress bars are redirected to m_stdout_os, which points to the
  //  original standard output stream.
  //  - All std::cout output is redirected to m_log_streambuf, which
  //  incorporates a LineInsertingFilter to continually shift the progress bar
  //  display downward.
...
/**
 * @brief A controller class that manages the display of progress bars that used
 * by MonitorStage.
 */
template <typename MessageT> class MonitorController {
public:
  /**
   * @brief Construct a new Monitor Controller object
   *
   * @param description : A text label displayed on the left side of the
   * progress bars
   * @param unit : the unit of message count
   * @param determine_count_fn : A function that computes the count for each
   * incoming message
   */
  MonitorController(
      const std::string &description, std::string unit = "messages",
      indicators::Color text_color = indicators::Color::cyan,
      indicators::FontStyle font_style = indicators::FontStyle::bold,
...
private:
  ...
  static std::string format_throughput(std::chrono::seconds duration,
                                       size_t count, const std::string &unit);
  ...
  bool m_is_started{
      false}; // Set to true after the first call to progress_sink()
};

template <typename MessageT>
MonitorController<MessageT>::MonitorController(
    const std::string &description, std::string unit,
    indicators::Color text_color, indicators::FontStyle font_style,
    std::optional<std::function<size_t(MessageT)>> determine_count_fn)
    : m_unit(std::move(unit)), m_determine_count_fn(determine_count_fn) {
  if (!m_determine_count_fn) {
    m_determine_count_fn = auto_count_fn();
    if (!m_determine_count_fn) {
      throw std::runtime_error(
          "No count function provided and no default count function available");
    }
  }

  m_bar_id = ProgressBarContextManager::get_instance().add_progress_bar(
      description, text_color, font_style);
}
...
MessageT MonitorController<MessageT>::progress_sink(MessageT msg) {
  ...
  pbar->set_option(indicators::option::PostfixText{
      format_throughput(duration, m_count, m_unit)});
  pbar->tick();
...
template <typename MessageT>
std::string
MonitorController<MessageT>::format_duration(std::chrono::seconds duration) {
  auto minutes = std::chrono::duration_cast<std::chrono::minutes>(duration);
  auto seconds = duration - minutes;

  std::ostringstream oss;
  oss << std::setw(2) << std::setfill('0') << minutes.count()
      << "m:" << std::setw(2) << std::setfill('0') << seconds.count() << "s";
  return oss.str();
}

template <typename MessageT>
std::string MonitorController<MessageT>::format_throughput(
    std::chrono::seconds duration, size_t count, const std::string &unit) {
...
template <typename MessageT>
auto MonitorController<MessageT>::auto_count_fn()
    -> std::optional<std::function<size_t(MessageT)>> {
  if constexpr (std::is_same_v<MessageT, std::shared_ptr<MessageMeta>>) {
    return [](std::shared_ptr<MessageMeta> msg) { return msg->count(); };
  }
